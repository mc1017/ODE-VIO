...
parser.add_argument('--train_seq', type=list, default=['04', '10'], help='sequences for training')
parser.add_argument('--val_seq', type=list, default=['04', '10'], help='sequences for validation')
...
def train(model, optimizer, train_loader, selection, temp, logger, ep, p=0.5, weighted=False):
    ...
    for i, (imgs, imus, gts, rot, weight, timestamps, folder) in enumerate(train_loader):
...
def main():
    ...
    batch_sampler = SequenceBoundarySampler(args.data_dir, batch_size=args.batch_size, train_seqs=args.train_seq, seq_len=args.seq_len)
    ...
    if args.pretrain_flownet and args.pretrain is None:
        ...
        print("Pretrained flownet loaded")
