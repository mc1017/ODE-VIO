...
class KITTI_tester:
    ...
    def test_one_path(self, net, df, selection, num_gpu=1, p=0.5):
        ...
        for i, (image_seq, imu_seq, gt_seq) in tqdm(
            ...
            with torch.no_grad():
                pose, decision, probs, hc = net(
                    x_in, i_in, is_first=(i == 0), hc=hc, selection=selection, p=p
                )
...
def plotPath_2D(
    seq, poses_gt_mat, poses_est_mat, plot_path_dir, decision, speed, window_size
):
