...
class PoseODERNN(nn.Module):
    ...
    def forward(self, fv, fv_alter, fi, dec, ts, prev=None, do_profile=False):
        ...
        new_hidden_states = self.rnn_cell(fused_features.squeeze(1), new_hidden_states)
        pose = self.regressor(new_hidden_states - initial_hidden_states)
        # pose.shape = [16, 6]
        return pose.unsqueeze(1), new_hidden_states


# Training 11 sequences at once instead of 1 sequence at a time
class PoseODERNN_V2(nn.Module):
    def __init__(self, opt):
        super(PoseODERNN_V2, self).__init__()
    ...
    def forward(self, fv, fv_alter, fi, dec, ts, prev=None, do_profile=False):
        ...
        # fused_features.shpae = [batch_size, seq_len, 768], 768 = 512 (image) + 256 (IMU)
        batch_size, seq_len, _ = fused_features.shape
        ...
        # Since we aim to find relative pose change, we stack the new_hidden_states with the initial_hidden_states
        stacked_states = torch.cat((initial_hidden_states, new_hidden_states), dim=1)
        pose = self.regressor(stacked_states)
        # pose.shape = [16, 6]
        return pose.unsqueeze(1), new_hidden_states
